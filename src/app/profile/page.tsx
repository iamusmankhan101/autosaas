...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (settings) {
      ...
      setInvoiceHeading(headingSetting?.value || "Auto Pulse Workshop");

      const footerSetting = settings.find(s => s.key === 'invoice_footer');
      setInvoiceFooter(footerSetting?.value || "Auto Pulse · Your Digital Workshop Companion");
    }
  }, [settings]);
      ...
      <Tabs defaultValue={defaultTab} className="space-y-10">
        <div className="flex items-center justify-center">
            <TabsList className="bg-slate-100 dark:bg-slate-800 p-1.5 rounded-full h-14 border border-slate-200 dark:border-slate-700">
                ...
                <TabsTrigger value="branches" className="rounded-full px-8 h-11 text-sm font-bold data-[state=active]:bg-white dark:data-[state=active]:bg-slate-900 data-[state=active]:shadow-lg data-[state=active]:text-orange-600 transition-all flex items-center gap-2">
                        ...
                        <CardContent className="p-8 md:p-10 pt-4 space-y-8">
                            <form onSubmit={handleSave} className="space-y-8">
                                ...
                                <div className="pt-4 flex flex-col md:flex-row gap-4 items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-8">
                                    <p className="text-xs text-slate-400 font-medium max-w-[250px] text-left">Your identity is used for official invoices and reports generated by Auto Pulse.</p>
